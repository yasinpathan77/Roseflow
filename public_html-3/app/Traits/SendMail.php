            ...
            if ($email_template && $email_template->is_active == 1) {
                if (app('general_setting')->mail_protocol == "smtp") {
                    ...
                    if (in_array("admin", json_decode($email_template->reciepnt_type))) {
                        ...
                        $superadmin_email = User::where('role_id', 1)->first();
                        
                        foreach ($order->packages as $key => $package) {
                            if ($superadmin_email->email && $key == 0) {
                                $superadmin_fullname = $superadmin_email->first_name ." ". $superadmin_email->last_name;
                                $datas = $this->mailData($email_template, $superadmin_fullname, $superadmin_email->email, $package->package_code);
                                $datas['attach'] = $path;
                                Mail::to($superadmin_email->email)->queue(new SendQueueMail($datas));
                            }
                        }
                    }
                    //End 007-add
