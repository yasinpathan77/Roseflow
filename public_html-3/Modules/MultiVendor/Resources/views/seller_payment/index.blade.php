                        ...
                        <div class="d-block">
                            <h3>{{ __('common.price') }}</h3>
                             <h1 class="gradient-color2 total_orders">
                                 ...
                                 {{ single_price($subscription->pricing->monthly_cost)}} (Monthly)
                                 @else
                                    {{single_price($subscription->pricing->yearly_cost) }} (Yearly)
                                 @endif
                            </h1>
                        </div>
                    </div>
                            ...
                            <h3>{{ __('seller.last_payment_date') }}</h3>
                             ...
                             <h1 class="gradient-color2 total_orders">
                                @if($subscription->is_paid == 1 && $subscription->last_payment_date != null)
                                    {{date(app('general_setting')->dateFormat->format, strtotime(Carbon\Carbon::createFromFormat('Y-m-d', $subscription->last_payment_date)))}}
                                @else
                                    ...
                                    <tbody>
                                        @if (isset($subscription))
                                            @foreach ($subscription_payment as $key => $payment)
                                                <tr>
                                                    <td class="nowrap">{{ date(app('general_setting')->dateFormat->format, strtotime($payment->created_at)) }}</td>
                                                    <td>{{ $payment->title }}</td>
