...
<div class="row">
    <div class="col-lg-12">
        <table class="table Crm_table_active3_slider_2">
            <thead>
                <tr>
            ...
            <tbody id="sku_tbody">
                @if(isset($header_2) && !empty($header_2))
                    @foreach($header_2 as $key => $header)
                    <tr>
                        <td>{{$key + 1}}</td>
                        <td>{{$header->name}}</td>
                            ...
                            <div class="dropdown CRM_dropdown">
                                ...
                                <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenu2">
                                    @if (permissionCheck('appearance.header.setup'))
                                        <a href="{{route('appearance.header.setup_2',$header->id)}}" class="dropdown-item edit_brand">{{__('common.setup')}}</a>
                                    @endif
                                </div>
                            </div>
