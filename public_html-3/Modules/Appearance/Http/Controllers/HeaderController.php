...
class HeaderController extends Controller
{
    ...
    public function index()
    {
        try{
            ...
            $data['header_2'] = $this->headerService->getByIn(array(5,6,7,8,9,10,11));
    ...
    public function setup_2($id){

        try{
            $data['header'] = $this->headerService->getById($id);
            $data['ZoneLists'] = $this->headerService->getAllZones();
            return view('appearance::header.components.setup_2',$data);
        }catch(Exception $e){
            LogActivity::errorLog($e->getMessage());
            Toastr::error(__('common.operation_failed'));
            return $e->getMessage();
    ...
    public function addElement_2(Request $request){

        try{
            $this->headerService->addElement_2($request->except('_token'));
            $data['header'] = $this->headerService->getById($request->id);
            $data['id'] = $request->id;
            LogActivity::successLog('element added successful.');
            return view('appearance::header.components.element_list_2',$data);
        }catch(Exception $e){
            LogActivity::errorLog($e->getMessage());
            Toastr::error(__('common.operation_failed'));
            return response()->json([
    ...
    public function updateElement_2(Request $request){
        
        try{
            $this->headerService->updateElement_2($request->except('_token'));
            $data['header'] = $this->headerService->getById($request->header_id);
    ...
    public function deleteElement_2(Request $request){
        try{
            $this->headerService->deleteElement_2($request->except('_token'));
